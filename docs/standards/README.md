# Standards

This directory stores the active shared platform standards.

Use it when the question is:

- what is the required enterprise process here
- which control model is current
- where approval, rollout, service, or observability expectations are defined

## Core Governance Standards

- [enterprise-workflow-model.md](enterprise-workflow-model.md)
- [governed-ai-access-model.md](governed-ai-access-model.md)
- [governed-runtime-lifecycle-model.md](governed-runtime-lifecycle-model.md)
- [review-and-approval-model.md](review-and-approval-model.md)
- [governed-change-model.md](governed-change-model.md)

## Shared Platform Standards

- [artifact-build.md](artifact-build.md)
- [ci-cd.md](ci-cd.md)
- [gitops.md](gitops.md)
- [observability.md](observability.md)
- [release-model.md](release-model.md)
- [restart-survival.md](restart-survival.md)
- [secrets.md](secrets.md)
- [service-contracts.md](service-contracts.md)
- [source-repo-contracts.md](source-repo-contracts.md)
- [version-attestation.md](version-attestation.md)

## Documentation And Boundary Standards

- [product-boundaries.md](product-boundaries.md)
- [product-documentation-model.md](product-documentation-model.md)

## Rule

If a shared control or operator expectation changes, update the relevant
standard here in the same change.
