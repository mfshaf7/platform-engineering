...
RUNNER_SCRIPT="${REPO_ROOT}/products/openproject/scripts/openproject_check_delivery_closeout_readiness_runner.rb"
...
if [[ -z "${TARGET_EPIC_ID}" ]]; then
  echo "TARGET_EPIC_ID is required, for example: make openproject-check-delivery-closeout-readiness TARGET_EPIC_ID=38" >&2
  exit 1
fi
...
echo "Checking closeout readiness for delivery epic ${TARGET_EPIC_ID} in ${OPENPROJECT_DELIVERY_PROJECT_IDENTIFIER}"

pod_name="$(openproject_pod)"
runner_remote="/tmp/openproject_check_delivery_closeout_readiness_runner.rb"
