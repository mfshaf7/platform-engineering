...
RUNNER_SCRIPT="${REPO_ROOT}/products/openproject/scripts/openproject_record_inspect_and_adapt_runner.rb"
...
if [[ -z "${TARGET_EPIC_ID}" ]]; then
  echo "TARGET_EPIC_ID is required, for example: make openproject-record-inspect-and-adapt TARGET_EPIC_ID=38 INSPECT_SUMMARY='PI review complete' ACTION_ITEMS='- Continue broker API migration'" >&2
  exit 1
fi
...
echo "Recording inspect-and-adapt entry for epic ${TARGET_EPIC_ID} in ${OPENPROJECT_DELIVERY_PROJECT_IDENTIFIER}"

pod_name="$(openproject_pod)"
runner_remote="/tmp/openproject_record_inspect_and_adapt_runner.rb"
