#!/usr/bin/env bash
set -euo pipefail

KUBECTL="${KUBECTL:-k3s kubectl}"
OPENPROJECT_NAMESPACE="${OPENPROJECT_NAMESPACE:-${BROKER_NAMESPACE:-openproject}}"
OPENPROJECT_DEPLOYMENT="${OPENPROJECT_DEPLOYMENT:-openproject-web}"
OPENPROJECT_DELIVERY_PROJECT_IDENTIFIER="${OPENPROJECT_DELIVERY_PROJECT_IDENTIFIER:-workspace-delivery-art}"
TARGET_EPIC_ID="${TARGET_EPIC_ID:-}"
TARGET_PI="${TARGET_PI:-}"
PI_REVIEW_DATE="${PI_REVIEW_DATE:-}"
PI_REVIEW_FILE="${PI_REVIEW_FILE:-}"

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
RUNNER_SCRIPT="${REPO_ROOT}/products/openproject/scripts/openproject_record_pi_review_runner.rb"
SUPPORT_SCRIPT="${REPO_ROOT}/products/openproject/scripts/openproject_delivery_art_custom_field_support.rb"

need_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "Missing required command: $1" >&2
    exit 1
  fi
}

kubectl_cmd() {
  ${KUBECTL} "$@"
}

openproject_pod() {
  kubectl_cmd -n "${OPENPROJECT_NAMESPACE}" get pod \
    -l "app.kubernetes.io/component=web,app.kubernetes.io/name=openproject" \
    -o jsonpath='{.items[0].metadata.name}'
}

need_cmd "${KUBECTL%% *}"

if [[ -z "${TARGET_EPIC_ID}" ]]; then
  echo "TARGET_EPIC_ID is required, for example: make openproject-record-pi-review TARGET_EPIC_ID=38 TARGET_PI=PI-2026-02 PI_REVIEW_FILE=/abs/path/pi-review.json" >&2
  exit 1
fi

if [[ -z "${PI_REVIEW_FILE}" ]]; then
  echo "PI_REVIEW_FILE is required, for example: make openproject-record-pi-review TARGET_EPIC_ID=38 TARGET_PI=PI-2026-02 PI_REVIEW_FILE=/abs/path/pi-review.json" >&2
  exit 1
fi

if [[ ! -f "${PI_REVIEW_FILE}" ]]; then
  echo "Missing PI review file: ${PI_REVIEW_FILE}" >&2
  exit 1
fi

if [[ ! -f "${RUNNER_SCRIPT}" ]]; then
  echo "Missing runner script: ${RUNNER_SCRIPT}" >&2
  exit 1
fi

if [[ ! -f "${SUPPORT_SCRIPT}" ]]; then
  echo "Missing support script: ${SUPPORT_SCRIPT}" >&2
  exit 1
fi

echo "Recording PI review from ${PI_REVIEW_FILE} for epic ${TARGET_EPIC_ID}"

pod_name="$(openproject_pod)"
runner_remote="/tmp/openproject_record_pi_review_runner.rb"
review_remote="/tmp/openproject_pi_review.json"
support_remote="/tmp/openproject_delivery_art_custom_field_support.rb"

kubectl_cmd -n "${OPENPROJECT_NAMESPACE}" cp "${RUNNER_SCRIPT}" "${pod_name}:${runner_remote}"
kubectl_cmd -n "${OPENPROJECT_NAMESPACE}" cp "${PI_REVIEW_FILE}" "${pod_name}:${review_remote}"
kubectl_cmd -n "${OPENPROJECT_NAMESPACE}" cp "${SUPPORT_SCRIPT}" "${pod_name}:${support_remote}"

cleanup() {
  kubectl_cmd -n "${OPENPROJECT_NAMESPACE}" exec "${pod_name}" -- rm -f "${runner_remote}" "${review_remote}" "${support_remote}" >/dev/null 2>&1 || true
}
trap cleanup EXIT

kubectl_cmd -n "${OPENPROJECT_NAMESPACE}" exec "${pod_name}" -- env \
  TARGET_EPIC_ID="${TARGET_EPIC_ID}" \
  TARGET_PI="${TARGET_PI}" \
  PI_REVIEW_DATE="${PI_REVIEW_DATE}" \
  OPENPROJECT_DELIVERY_PROJECT_IDENTIFIER="${OPENPROJECT_DELIVERY_PROJECT_IDENTIFIER}" \
  sh -lc 'bundle exec rails runner "$1" "$2"' sh "${runner_remote}" "${review_remote}"
