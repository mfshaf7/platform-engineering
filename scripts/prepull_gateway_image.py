...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Pre-pull the current gateway image onto every schedulable node before rollout."
    )
    ...
    namespace = ENV_TO_NAMESPACE[args.environment]
    name = f"openclaw-gateway-prepull-{args.environment}"
    kubectl = args.kubectl.split()
    ...
    try:
        ...
        run([*kubectl, "-n", namespace, "delete", "daemonset", name, "--ignore-not-found"])
